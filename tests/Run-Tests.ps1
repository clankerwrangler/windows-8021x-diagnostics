...
$loggingHash = (Get-FileHash -LiteralPath (Join-Path (Split-Path $ScriptPath -Parent) 'Set-8021xLogging.ps1') -Algorithm SHA256).Hash.ToLowerInvariant()
Write-Output ('RECEIPT logging_sha256=' + $loggingHash)
...
try {
    ...
    foreach ($name in @('Test-Static.ps1','Test-Harness.ps1','Test-Rules.ps1','Test-ErrorCodes.ps1','Test-Report.ps1','Test-Xml.ps1','Test-Collection.ps1','Test-Process.ps1','Test-IO.ps1','Test-WiredOwnership.ps1','Test-Logging.ps1')) {
        $code = '& ' + (ConvertTo-TestLiteral (Join-Path $PSScriptRoot $name)) + ' -ScriptPath ' + (ConvertTo-TestLiteral $ScriptPath)
        if ($name -in @('Test-Process.ps1','Test-IO.ps1','Test-WiredOwnership.ps1','Test-Logging.ps1')) { $code += ' -ScratchPath ' + (ConvertTo-TestLiteral $ScratchPath) }
